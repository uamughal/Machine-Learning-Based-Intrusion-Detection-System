...
def create_model(neurons, act_f, layers, loss_func, ki):

    model = Sequential()
    model.add(LSTM(int(neurons), kernel_initializer=ki, activation=act_f,
                   batch_input_shape=(None, WS, feats), return_sequences=True))
    if layers == 2:
        model.add(LSTM(int(neurons * 2/3), kernel_initializer=ki, activation=act_f, return_sequences=False))
        model.add(RepeatVector(WS))
        model.add(LSTM(int(neurons * 2/3), kernel_initializer=ki, activation=act_f, return_sequences=True))
        model.add(LSTM(int(neurons), kernel_initializer=ki, activation=act_f, return_sequences=True))

    elif layers == 4:
        model.add(LSTM(int(neurons * 3/4), kernel_initializer=ki, activation=act_f, return_sequences=True))
        model.add(LSTM(int(neurons * 2/4), kernel_initializer=ki, activation=act_f, return_sequences=False))
        model.add(RepeatVector(WS))
        model.add(LSTM(int(neurons * 2/4), kernel_initializer=ki, activation=act_f, return_sequences=True))
        model.add(LSTM(int(neurons * 3/4), kernel_initializer=ki, activation=act_f, return_sequences=True))
        model.add(LSTM(int(neurons), kernel_initializer=ki, activation=act_f, return_sequences=True))

    elif layers == 6:
        model.add(LSTM(int(neurons * 8 / 9), kernel_initializer=ki, activation=act_f, return_sequences=True))
        model.add(LSTM(int(neurons * 7 / 9), kernel_initializer=ki, activation=act_f, return_sequences=True))
        model.add(LSTM(int(neurons * 6 / 9), kernel_initializer=ki, activation=act_f, return_sequences=False))
        model.add(RepeatVector(WS))
        model.add(LSTM(int(neurons * 6 / 9), kernel_initializer=ki, activation=act_f, return_sequences=True))
        model.add(LSTM(int(neurons * 7 / 9), kernel_initializer=ki, activation=act_f, return_sequences=True))
        model.add(LSTM(int(neurons * 8 / 9), kernel_initializer=ki, activation=act_f, return_sequences=True))
        model.add(LSTM(int(neurons), kernel_initializer=ki, activation=act_f, return_sequences=True))

    elif layers == 8:
        model.add(LSTM(int(neurons * 14 / 16), kernel_initializer=ki, activation=act_f, return_sequences=True))
        model.add(LSTM(int(neurons * 12 / 16), kernel_initializer=ki, activation=act_f, return_sequences=True))
        model.add(LSTM(int(neurons * 10 / 16), kernel_initializer=ki, activation=act_f, return_sequences=True))
        model.add(LSTM(int(neurons * 8 / 16), kernel_initializer=ki, activation=act_f, return_sequences=False))
        model.add(RepeatVector(WS))
        model.add(LSTM(int(neurons * 8 / 16), kernel_initializer=ki, activation=act_f, return_sequences=True))
        model.add(LSTM(int(neurons * 10 / 16), kernel_initializer=ki, activation=act_f, return_sequences=True))
        model.add(LSTM(int(neurons * 12 / 16), kernel_initializer=ki, activation=act_f, return_sequences=True))
        model.add(LSTM(int(neurons * 14 / 16), kernel_initializer=ki, activation=act_f, return_sequences=True))
        model.add(LSTM(int(neurons), kernel_initializer=ki, activation=act_f, return_sequences=True))
    ...
    model.compile(loss=loss_func, optimizer='adam', metrics=['MeanSquaredError', 'mean_absolute_error', 'AUC',
...
for train, test in kf.split(X, Y):
    ...
    model = create_model(neurons, act_f, hiddenlayers, 'mse', ki)
    early_stop = tf.keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=1e-2, patience=5, verbose=1, mode='auto',
                                              baseline=None, restore_best_weights=True)

    history = model.fit(x=X[train], y=Y[train], validation_data=(X[test], Y[test]), epochs=20, batch_size=64, callbacks=[early_stop]).history
